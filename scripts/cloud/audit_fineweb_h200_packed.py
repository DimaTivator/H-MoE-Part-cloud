...
def main() -> int:
    ...
    for rank_metadata in metadata["ranks"]:
        ...
        path = args.dataset / rank_metadata["file"]
        assert path.stat().st_size == int(rank_metadata["bytes"])
        digest = sha256_file(path)
        assert digest == rank_metadata["sha256"]
        ...
        print(
            f"rank={rank} blocks={rank_metadata['blocks']} bytes={path.stat().st_size} "
            f"sha256={digest} preview_sha256={preview_digest}",
            flush=True,
        )
